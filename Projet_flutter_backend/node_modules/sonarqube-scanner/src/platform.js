...
function isAlpineLinux() {
    ...
    try {
        ...
        (0, logging_1.log)(logging_1.LogLevel.DEBUG, `Failed to read /etc/os-release, trying /usr/lib/os-release: ${error}`);
        try {
            const fileContent = fs.readFileSync('/usr/lib/os-release');
            content = fileContent.toString();
        }
        catch (secondError) {
            (0, logging_1.log)(logging_1.LogLevel.WARN, `Failed to read /etc/os-release or /usr/lib/os-release: ${secondError}`);
        }
    }
    const match = /^ID=([^\r\n]*)/m.exec(content ?? '');
